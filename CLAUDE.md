...
- **Design system:** Gift-wrap theme — `.gift-box-wrap` (amber wrapping paper bg), `.envelope-card` (kraft envelope contributor cards), `.greeting-card-frame` (cream content container), `.ribbon-divider`, red ribbon overlays, `.wax-seal`. Amber/gold accents, Playfair Display + Sora fonts. Admin UI uses `.nord-card`, `.editorial-line`, `.input-field`.
